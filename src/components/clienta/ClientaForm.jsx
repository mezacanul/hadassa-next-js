...
export default function ClientaForm({ clienta }) {
    const [fullNombre, setFullNombre] = useState(`${clienta.nombres} ${clienta.apellidos}`)
    ...
    return (
        <VStack alignItems={"start"} w={"60%"} gap={"1rem"} mb={"2rem"}>
            <Heading color={"pink.600"} mb={"1rem"} size={"3xl"}>{`${fullNombre}`}</Heading>
            <FotoPerfilInput clienta={clienta}/>
            <DatosForm clienta={clienta} setFullNombre={setFullNombre}/>
        </VStack>
    )
}