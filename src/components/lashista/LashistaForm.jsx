import { Box, Button, Grid, HStack, Input, Spinner, Text, VStack } from "@chakra-ui/react";
...
export default function LashistaForm({ lashista }) {
    ...
    useEffect(() => {
        setLashistaForm({
            ...
            // Array - [ "HH:mm", "HH:mm" ]
            horarioSBD: lashista.horarioSBD.split("-").map((hora) => (hora.replace(" ", "")))
        })
    }, [])
    ...
    return (
        <VStack alignItems={"start"} w={"50%"} gap={"1.5rem"} mb={"5rem"}>
            <Grid w={"100%"} gridTemplateColumns={"1fr 1fr"} gap={"1rem"}>
                <InputGroup
                    label={"Nombre"}
                    value={lashistaForm.nombre}
                    onChange={(e) => {
                        setLashistaForm({ ...lashistaForm, nombre: e.target.value })
                    }}
                />
                <InputGroup
                    label={"Correo"}
                    ...
                    onChange={(e) => {
                        setLashistaForm({ ...lashistaForm, email: e.target.value })
                    }}
                />
                <InputGroup
                    label={"Contraseña"}
                    ...
                    onChange={(e) => {
                        setLashistaForm({ ...lashistaForm, password: e.target.value })
                    }}
                />
            ...
            <VStack alignItems={"start"} w={"100%"}>
                <Text mb={"0.5rem"} fontWeight={600} fontSize={"0.8rem"}>{"Horario de Lunes a Viernes"}</Text>
                <VStack w={"100%"} gap={"1.5rem"}>
                    {lashistaForm.horarioLV && lashistaForm.horarioLV.map((horarios, i) => (
                        <TimeSelector
            ...
            <VStack alignItems={"start"} w={"100%"}>
                <Text mb={"0.5rem"} fontWeight={600} fontSize={"0.8rem"}>{"Horario Sábado"}</Text>
                <HStack w={"100%"}>
                    {lashistaForm.horarioSBD && lashistaForm.horarioSBD.map((hora, i) => (
                        <TimePickMUI
                            key={i}
                            label={"Seleccionar hora"}
                            value={hora}
...
function TimeSelector({ label, horarios, lashistaForm, setLashistaForm, index }) {
    //Array of [ "HH:mm", ... ]
    const [horariosArr, setHorariosArr] = useState(horarios.split("-").map((hora) => (hora.replace(" ", ""))))
...
function InputGroup({ label, value, onChange }) {
    return (
        <VStack alignItems={"start"}>
            <Text fontWeight={600} fontSize={"0.8rem"}>{label}</Text>
            <Input
                {...inputStyles}
                value={value}
                placeholder={label}
